...
// src/app/use-cases/cause/get-causes-by-userId.spec.ts
...
describe("Create cause", () => {
  it("should be able to create a cause", async () => {
    ...
    const getCausesByUserId = new GetCausesByUserId(repository);
    ...
    const { causes } = await getCausesByUserId.execute({
      userId: cause.userId
    });
    globalExpect(cause).toBeTruthy();
    globalExpect(causes).toBeTruthy();
    globalExpect(causes[0].userId).toEqual(cause.userId);
  });
});
